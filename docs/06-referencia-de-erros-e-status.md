...
|---|---|---|
| `400 Bad Request` | `POST /flows`, `PUT /flows/{id}`, `GET/POST /flows/{id}/executions`, `POST /flows/{flow_id}/execute` e `POST /webhooks/signature` (Runner) | Body ou parâmetros inválidos (inclui `channel` fora de `whatsapp`/`api`). **Validado no Sequencer desde 21/08/2026:** `settings.deadline_at` fora do aceito (não inteiro positivo, ou inteiro acima do teto de 90 dias, ou string fora do ISO 8601) e `documents[].kind = "runner_files"` sem `key`/`filename` |
...
| `409 Conflict` | `PUT /flows/{id}`, `DELETE /flows/{id}` *(novo, 21/08/2026)*, `PATCH /flows/{id}/publish`, `PATCH /flows/{id}/unpublish`, `POST /executions/{execution_id}/next` | Flow já está no status alvo, ou execução não está em um estado que permite avançar. **`DELETE /flows/{id}`**: exclusão recusada porque o Flow tem execução em andamento — a mesma regra que trava a despublicação (o Flow só pode ser excluído com as transações finalizadas; a exclusão é lógica e preserva o histórico). **`PUT /flows/{id}` ganhou esse código numa geração de contrato mais recente (15/07/2026) sem detalhar a causa** — distinto do `403` (que já é conhecido: editar um Flow `published`); não confirmado com o time técnico o que dispara especificamente o `409` aqui |
