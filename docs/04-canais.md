...
> **⚠️ Enum incompleto observado.** O spec do Runner declara `current_step.type` com apenas `form`, `verify`, `signature`, `acceptance` — sem `kyc` e sem `consent`, os dois tipos mais novos (ver [`02-conceitos-e-modelo-de-dados.md`](02-conceitos-e-modelo-de-dados.md)). Não está confirmado com o time técnico o que o canal `api` devolve como `current_step` quando o passo em andamento é de um desses dois tipos. Hipóteses plausíveis, nenhuma verificada: `kyc` é resolvido automaticamente, sem etapa visível ao contato; e `consent`, cujo `channel_type` só aceita `whatsapp`, se comportaria como `acceptance` (sem `url` a renderizar). **Valide antes de depender do canal `api` num Flow que use `kyc` ou `consent`.**
>
> Consequência prática do mesmo ponto: como o `context` de um step `consent` exige `channel_type: "whatsapp"`, a expectativa é que ele também precise de `contact.phone_number` mesmo no canal `api` — a mesma exceção já documentada para `acceptance`. O contrato do Runner ainda menciona só `acceptance` nessa exceção, então trate isso como comportamento a confirmar, não como regra publicada.
